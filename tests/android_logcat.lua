...
vim.fn.writefile({
  '#!/usr/bin/env python3',
  'import sys,time',
  "assert sys.argv[1:]==['-s','test-device','logcat','--pid=123','-v','brief','-T','1']",
...
assert(
  vim.wait(3000, function()
    return appended:find('hello from logcat', 1, true) ~= nil
  end),
  'logcat never reached REPL'
)
dap.listeners.after.disconnect['android-logcat'](session)
local snapshot = appended
vim.wait(200, function()
  return false
end)
assert(appended == snapshot, 'output continued after disconnect')
session.config.android.logcat = false
dap.listeners.after.event_initialized['android-logcat'](session)
vim.wait(200, function()
  return false
end)
assert(appended == snapshot, 'logcat=false must disable streaming')
